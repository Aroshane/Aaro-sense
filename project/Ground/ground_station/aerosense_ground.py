...
# ── Config ────────────────────────────────────────────────────────────────────
# Serial settings for Arduino Nano telemetry link
LORA_SERIAL_PORT = "COM3"      # Change to your Arduino Nano's COM port (e.g., "/dev/ttyUSB0" on Linux)
LORA_SERIAL_BAUD = 115200      # Must match Serial.begin(115200) in Arduino Nano
...
# ── Serial Telemetry Receiver Thread ──────────────────────────────────────────
def serial_listener_thread(port, baudrate, packet_queue):
    try:
        import serial
    except ImportError:
        log.error("pyserial is not installed! Run 'pip install pyserial' to use the Arduino Nano USB link.")
        return

    log.info(f"Serial listener thread targeting {port} @ {baudrate} baud")
    ser = None
    buffer = bytearray()
    ...
    while running:
        if ser is None:
            try:
                ...
                log.warning(f"Could not open serial port {port}: {e}. Retrying in 5 seconds...")
                time.sleep(5)
                continue
        ...
        try:
            ...
            if ser.in_waiting > 0:
                ...
                # Read with timeout to avoid high CPU load
                chunk = ser.read(1)
                if chunk:
                    buffer.extend(chunk)
                else:
            ...
            # Process buffer
            while len(buffer) >= 65:
                # Look for header sequence: 0x9D followed by 0xCB
                start_idx = -1
                for i in range(len(buffer) - 1):
                    if buffer[i] == 0x9D and buffer[i+1] == 0xCB:
                        start_idx = i
                        break
                
                if start_idx == -1:
                    # Clear buffer keeping only the last byte if it is 0x9D
                    if len(buffer) > 0 and buffer[-1] == 0x9D:
                        buffer = buffer[-1:]
                    else:
                        buffer.clear()
                    break
                ...
                try:
                    # Verify by unpacking
                    decoded = msgpack.unpackb(pkt)
                    if isinstance(decoded, list) and len(decoded) == 13:
                        packet_queue.append((pkt, "lora_serial", 0))
                        buffer = buffer[65:]
                        continue
                except Exception:
                    pass
                
                # Slide past the invalid byte
                buffer = buffer[1:]
                
        except Exception as e:
            log.error(f"Serial read exception: {e}")
            try:
                ser.close()
            except Exception:
                pass
...
def main():
    ...
    # 3. Start Serial Receiver Thread
    serial_thread = threading.Thread(target=serial_listener_thread, args=(LORA_SERIAL_PORT, LORA_SERIAL_BAUD, packet_queue))
    serial_thread.daemon = True
    serial_thread.start()

    # 4. Start UDP Receiver Thread
