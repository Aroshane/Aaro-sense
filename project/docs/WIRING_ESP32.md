...
| **PMS5003** (Laser Particle Box) | VCC (5V) / GND / TXD / RXD | **5V BEC Rail** / **GND Rail** / **GPIO26 (RX2)** / **GPIO27 (TX2)** | Teal (Hardware UART2) | Laser diagnostic tracking for suspended particulate matter (PM2.5 and PM10). |
...
## 🚫 Why VTX (Video Transmitter) & FPV Camera Are Excluded

During hardware consolidation, FPV analog cameras and Video Transmitters (VTX) were excluded from the AeroSense build. Here is why your project does not need them:

1. **Autonomous Waypoint Mission Execution**: AeroSense is designed as an autonomous environmental survey drone. The flight path is a pre-programmed lawnmower grid handled automatically by ArduPilot. You are not piloting the drone manually in first-person-view (FPV) through a live camera stream.
2. **Real-time Dashboard vs. Video Feed**: Your telemetry link (via 433 MHz LoRa or WiFi UDP) already acts as your "live feed," transmitting coordinates and sensor data points to the ground station Dash dashboard. Real-time graphs and 3D paths are the core mapping deliverables—video feeds contribute no value to air quality mapping.
3. **Payload Weight & Efficiency**: Removing the camera, VTX board, and whip antenna saves 15-30g of weight and eliminates heavy power consumption from the battery pack, directly translating to longer flight times for your survey.
4. **RF Interference & Regulatory Compliance**: VTX modules emit on the 5.8 GHz band. Eliminating them avoids RF noise on your control/LoRa links and simplifies compliance under civilian drone regulatory standards (like DGCA visual line-of-sight guidelines), which mandate pilot line-of-sight observation from the ground.

Therefore, do not purchase or mount these parts. All `CAM`, `VTX`, and `9V` terminals on your SpeedyBee autopilot board should be left completely unconnected.
